...
type UDPRouteLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha4.UDPRoute, err error)
...
// List lists all UDPRoutes in the indexer.
func (s *uDPRouteLister) List(selector labels.Selector) (ret []*v1alpha4.UDPRoute, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha4.UDPRoute))
	})
	return ret, err
}
...
type UDPRouteNamespaceLister interface {
	...
	Get(name string) (*v1alpha4.UDPRoute, error)
...
// List lists all UDPRoutes in the indexer for a given namespace.
func (s uDPRouteNamespaceLister) List(selector labels.Selector) (ret []*v1alpha4.UDPRoute, err error) {
...
// Get retrieves the UDPRoute from the indexer for a given namespace and name.
func (s uDPRouteNamespaceLister) Get(name string) (*v1alpha4.UDPRoute, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha4.Resource("udproute"), name)
	}
	return obj.(*v1alpha4.UDPRoute), nil
}
