...
type HTTPRoutesLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.HTTPRoutes, err error)
...
// List lists all HTTPRouteses in the indexer.
func (s *hTTPRoutesLister) List(selector labels.Selector) (ret []*v1alpha1.HTTPRoutes, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.HTTPRoutes))
	})
	return ret, err
}
...
type HTTPRoutesNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.HTTPRoutes, error)
...
// List lists all HTTPRouteses in the indexer for a given namespace.
func (s hTTPRoutesNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.HTTPRoutes, err error) {
...
// Get retrieves the HTTPRoutes from the indexer for a given namespace and name.
func (s hTTPRoutesNamespaceLister) Get(name string) (*v1alpha1.HTTPRoutes, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("httproutes"), name)
	}
	return obj.(*v1alpha1.HTTPRoutes), nil
}
