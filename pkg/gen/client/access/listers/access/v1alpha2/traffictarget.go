...
type TrafficTargetLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.TrafficTarget, err error)
...
// List lists all TrafficTargets in the indexer.
func (s *trafficTargetLister) List(selector labels.Selector) (ret []*v1alpha2.TrafficTarget, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.TrafficTarget))
	})
	return ret, err
}
...
type TrafficTargetNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.TrafficTarget, error)
...
// List lists all TrafficTargets in the indexer for a given namespace.
func (s trafficTargetNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.TrafficTarget, err error) {
...
// Get retrieves the TrafficTarget from the indexer for a given namespace and name.
func (s trafficTargetNamespaceLister) Get(name string) (*v1alpha2.TrafficTarget, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("traffictarget"), name)
	}
	return obj.(*v1alpha2.TrafficTarget), nil
}
